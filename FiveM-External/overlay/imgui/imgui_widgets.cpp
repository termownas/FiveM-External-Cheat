...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (mixed_value)
    {
        ...
        ImVec2 pad(ImMax(1.0f, IM_FLOOR(square_sz / 1.6f)), ImMax(1.0f, IM_FLOOR(square_sz / 3.6f)));
        window->DrawList->AddRectFilled(check_bb.Min + pad, check_bb.Max - pad, check_col, style.FrameRounding);
    }
    else if (*v)
    {
        const float pad = ImMax(5.0f, IM_FLOOR(square_sz / 6.0f));
...
window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(grab_bb.Min.x + 11, frame_bb.Max.y), ImColor(231, 97, 223), 2);
window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(1.0f, 1.0f, 1.0f, 0.05f + it_anim->second.outline_anim), 2);
    ...
    RenderText(total_bb.Min + ImVec2(145, 2), value_buf);
